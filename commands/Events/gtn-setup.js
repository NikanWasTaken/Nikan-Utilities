    ...
    run: async (client, message, args, wrongUsage) => {
        ...
        if (!databrr) {
            ...
            const confirmation = new MessageEmbed()
                .setTitle(`<:guess_the_number:903189197257592872> Confirmation`)
                .setDescription(`\`\`\`Are you sure that you want to start this setup and you want to host this guess the number event? Please type "yes" or "no" in the chat to continue!\`\`\``)
                .setColor("ORANGE")
                .setFooter("You have 2 minutes to use this, after this, the bot won't respond!")

            const firstmsg = message.member.send({
                embeds: [confirmation]
            })
                .catch(e => {
                    return message.channel.send("Looks like your dms are closed, you need to open them for the setup process!")
                })

            const dmchannel = (await firstmsg).channel;
            const confirmationcollector = dmchannel.createMessageCollector({
                time: 120000,
                min: 1,
                filter: (r) => r.channel.id !== message.channelId
            });
            ...
            confirmationcollector.on('collect', async (m) => {
                ...
                if (m.content.toLowerCase() == "cancel") {
                    return dmchannel.send("Canceled!"),
                        await database.findByIdAndDelete(`${eeee._id}`), confirmationcollector.stop()
                }

                if (m.content.toLowerCase() == "yes") {

                    const yes = new MessageEmbed()
                        .setDescription("Alright, Continuing the setup process!")
                        .setColor("GREEN")
                    ...
                    const meow = new MessageEmbed()
                        .setDescription("You can cancel the setup process whenever you want by simply writring `cancel` in the chat!")
                        .setColor("YELLOW")

                    const firstq = new MessageEmbed()
                        .setTitle("<:guess_the_number:903189197257592872> Question #1`")
                        .setDescription(`\`\`\`That is the numbers range? This means the correct number is between these 2 numbers!\nPlease use the correct format for the bot to detect these 2 numbers: "(firstnumber) / (secondNumber)". Example: 1 / 500\nDon't forget to put the space between slash and the 2 numbers!\`\`\``)
                        .setColor("ORANGE")
                        .setFooter("You have 2 minutes to use this, after this, the bot won't respond!")
                    ...
                    firstc.on('collect', async (m) => {
                        ...
                        if (m.content.toLowerCase() == "cancel") {
                            return dmchannel.send("Canceled!"),
                                await database.findByIdAndDelete(`${eeee._id}`),
                                firstc.stop()
                        }
                        const check = m.content.split(" / ")

                        database.findOneAndUpdate({
                            guildId: `${client.server.id}`, hostId: `${m.author.id}`
                        }, { $set: { range: `${check[0]} to ${check[1]}` } })

                        const alr = new MessageEmbed()
                            .setDescription(`Alright, the numbers range has been set to **${check[0]} to ${check[1]}**.`)
                            .setColor("YELLOW")
                        ...
                        const secondq = new MessageEmbed()
                            .setTitle("<:guess_the_number:903189197257592872> Question #2`")
                            .setDescription(`\`\`\`What is the correct number? The winner will be chosen if someone guess this number. Make sure that you're choosing the correct number because this can't be changed later!\`\`\``)
                            .setColor("ORANGE")
                            .setFooter("You have 2 minutes to use this, after this, the bot won't respond!")
                        ...
                        secondc.on('collect', async (m) => {
                            ...
                            if (isNaN(m.content))
                                return dmchannel.send("Invalid Number, please try again but this time provide a valid number!")


                            database.findOneAndUpdate({
                                guildId: `${client.server.id}`, hostId: `${m.author.id}`
                            }, { $set: { correctNumber: `${m.content}` } })

                            const yestwo = new MessageEmbed()
                                .setDescription(`Alright, The correct number has been set to **${m.content}**`)
                                .setColor("GREEN")
                            ...
                            const thirdq = new MessageEmbed()
                                .setTitle("<:guess_the_number:903189197257592872> Question #3`")
                                .setDescription(`\`\`\`What's the prize for this event?\`\`\``)
                                .setColor("ORANGE")
                                .setFooter("You have 2 minutes to use this, after this, the bot won't respond!")
                            ...
                            thirdc.on('collect', async (m) => {
                                ...
                                const yestwo = new MessageEmbed()
                                    .setDescription(`Alright, The prize has been set to **${m.content}**`)
                                    .setColor("GREEN")
                                ...
                                collector.on("collect", async (collected) => {
                                    ...
                                    if (collected.customId === "yes-setup-gtn") {
                                        ...
                                        database.findOneAndUpdate({
                                            guildId: `${client.server.id}`, hostId: `${collected.user.id}`
                                        }, { $set: { status: `saved and ready to get started` } })
                    ...
                    const no = new MessageEmbed()
                        .setDescription("Alright, I cancelled the set up process!")
                        .setColor("RED")
            ...
            const bla = new MessageEmbed()
                .setDescription(`This server has already data queued for the user: ${message.guild.members.cache.get(databrr.hostId)}\nThey need to start their event before you can queue your own one!`)
                .setColor("RED")
            message.reply({
                embeds: [bla]
            })
